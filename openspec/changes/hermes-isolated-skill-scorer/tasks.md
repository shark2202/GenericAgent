...
- [ ] 2.1 改 `distill()`(`plugins/skill_evolution.py:148-152`):在 `_parse_op` 与 `_apply_op` 之间插 `_scorer.score(op, skill_md, history, catalog)`
- [ ] 2.2 闸门逻辑:`verdict=='pass'` 且 `score>=阈值` → 继续 `_apply_op`;否则 `handler._pending_briefs.append(build_brief('rejected-by-scorer', name, rationale, path))` 且不落盘
...
- [ ] 3.1 `SubagentScorer` 构造 `desc`:候选 SKILL.md 全文 + `history_info[-40:]` 快照 + catalog 文本;**断言不含** `op.get('reason')` 与生成器 prompt 片段(单测 `assert 'reason' not in desc` 类)
- [ ] 3.2 `tools_subset` 限定 `['file_read']`(强制 `submit_result` 由 `run_single`/`_apply_task_mode` 注入);确认不给 `code_run`/`file_write`/`web_scan`/`file_patch`(单测断言 `GA_TASK_TOOLS` env 不含这些)
...
- [ ] 6.5 单测独立性:`SubagentScorer` 构造的 `desc` 不含 `op['reason']`;`tools_subset` 不含执行类工具
