...
def _strip_jsonc(text):
    ...
    State-machine parser — safe with comments inside strings."""
    out = []
    i = 0
    n = len(text)
    in_str = False
...
def _load_mykeys():
    ...
    # Fallback: mykey.jsonc (JSONC with comments) → mykey.json (plain JSON)
    _root = os.path.dirname(os.path.abspath(__file__))
    for _fname in ("mykey.jsonc", "mykey.json"):
        p = os.path.join(_root, _fname)
        ...
        mk = (
            ...
            if _fname.endswith(".jsonc")
            else json.loads(raw)
        )
        _mykey_path = p
        if isinstance(mk, dict) and "remote_url" in mk:
            return requests.get(mk["remote_url"], timeout=10).json()
        return mk
    raise Exception(
        '[ERROR] No config found: mykey.py / mykey.jsonc / mykey.json. Run "python configure_mykey.py" or copy mykey_template.jsonc to mykey.jsonc and fill in your keys.'
    )
...
def compress_history_tags(
    messages, keep_recent=10, max_len=800, force=False, interval=5
):
        ...
        if isinstance(c, str):
            ...
            for b in c:
                ...
                if t == "text" and isinstance(b.get("text"), str):
                    ...
                    b["thinking"] = _trunc_str(b["thinking"])
                elif t == "tool_result":
                    tc = b.get("content")
...
def _parse_claude_sse(resp_lines):
    ...
    if warn:
        ...
        insert_at = next(
            (i for i, b in enumerate(content_blocks) if b.get("type") == "tool_use"),
            len(content_blocks),
        )
        content_blocks.insert(insert_at, {"type": "text", "text": warn})
        yield warn
    return content_blocks
...
def _openai_stream(sess, messages):
    ...
    if api_mode == "responses":
        ...
        payload = {
            ...
            "client_metadata": {
                "x-codex-window-id": f"{_RESP_CACHE_KEY}:0",
                "x-codex-installation-id": _RESP_CODEX_KEY,
            },
    ...
    for m in merged:
        ...
        if m["role"] == "assistant":
            ...
            for b in c:
                ...
                if tid and tid in prev_uses and tid not in got:
                    got[tid] = b
                elif isinstance(b, dict) and b.get("type") == "tool_result":
                    rest.append({"type": "text", "text": str(b.get("content", ""))})
                else:
                    rest.append(b)
            m["content"] = [
                got.get(u)
                or {"type": "tool_result", "tool_use_id": u, "content": "(error)"}
                for u in prev_uses
            ] + rest
            prev_uses = []
    return merged
...
class NativeClaudeSession(BaseSession):
    ...
    def __init__(self, cfg):
        ...
        self.tools = None
        if self.user_agent == self.default_ua:
            self.user_agent = self.native_ua
...
class MixinSession:
    ...
    @property
    def model(self):
        return getattr(self._sessions[self._cur_idx], "model", None)

    @property
    def current_name(self):
        return getattr(self._sessions[self._cur_idx], "name", None)
...
class NativeToolClient:
    ...
    def chat(self, messages, tools=None):
        ...
        _write_llm_log(
            "Prompt", json.dumps(merged, ensure_ascii=False, indent=2), self.log_path
        )
        ...
        if resp:
            _write_llm_log(
                "Response", resp.raw, self.log_path, model=self.backend.model
            )
...
def resolve_session(cfg_name):
    ...
    if "native" in cfg_name:
        return (NativeClaudeSession if "claude" in cfg_name else NativeOAISession)(
            cfg=cfg
        )
...
def resolve_client(cfg_name):
    ...
    return (
        (
            NativeToolClient(s)
            if isinstance(s, (NativeClaudeSession, NativeOAISession))
            else ToolClient(s)
        )
        if s
        else None
